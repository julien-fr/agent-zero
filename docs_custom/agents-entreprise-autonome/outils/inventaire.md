# Inventaire des outils d'Agent Zero
...
Chaque agent spécialisé utilise un sous‑ensemble de ces outils en fonction de son rôle. Le mapping détaillé est disponible dans [mapping-agents.md](./mapping-agents.md).

## Évolution

Cet inventaire est mis à jour régulièrement avec l'ajout de nouveaux outils dans le framework Agent Zero. Pour les dernières modifications, consulter le répertoire `git/agent-zero/python/tools/`.
